...
class ServerManagementCog(commands.Cog):
    def __init__(self, bot: DiscoBunty):
        self.bot = bot
        self.state = bot.state
        if self.state.config.features.enable_docker:
            self.bot.tree.add_command(DockerGroup(self.bot))
        self.bot.tree.on_error = self.on_app_command_error

    @commands.Cog.listener()
    async def on_ready(self) -> None:
        self.state.logger.info("Logged in as %s (ID: %s)", self.bot.user, self.bot.user.id)
        self.state.logger.info("Docker integration: %s", "Enabled" if self.state.config.features.enable_docker else "Disabled")
        self.state.logger.info("------")

    async def on_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError) -> None:
        reference_id = secrets.token_hex(4)
        self.state.logger.exception("Discord app command error [%s]: %s", reference_id, error, exc_info=error)
    ...
    def ensure_server_access(self, interaction: discord.Interaction, server: str) -> None:
        if not check_permissions(self.state, interaction.user, server):
    ...
    async def _generic_autocomplete(
        ...
        if not check_permissions(self.state, interaction.user):
            return []

        if require_server:
            server = getattr(interaction.namespace, "server", None)
            if not server or not check_permissions(self.state, interaction.user, server):
                return []
    ...
    async def server_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
        async def fetcher():
            return [alias for alias in self.state.ssh_manager.get_server_aliases() if check_permissions(self.state, interaction.user, alias)]

        return await self._generic_autocomplete(interaction, current, fetcher)

    async def log_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
        async def fetcher():
            return await asyncio.to_thread(self.state.ssh_manager.get_log_files, interaction.namespace.server)

        return await self._generic_autocomplete(interaction, current, fetcher, require_server=True)

    @app_commands.command(name="ping", description="Check bot latency")
    async def ping(self, interaction: discord.Interaction) -> None:
        await interaction.response.send_message(f"Pong! {round(self.bot.latency * 1000)}ms")

    @app_commands.command(name="server", description="Server management commands")
    ...
    async def server_power(self, interaction: discord.Interaction, server: str, action: str) -> None:
    ...
    @app_commands.command(name="stats", description="Show system statistics for a server")
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    @app_commands.autocomplete(server=server_autocomplete)
    async def stats(self, interaction: discord.Interaction, server: str) -> None:
        self.ensure_server_access(interaction, server)
        await interaction.response.defer()
        output = await asyncio.to_thread(self.state.ssh_manager.get_system_stats, server)
        await interaction.followup.send(f"**System Stats for `{server}`:**\n```\n{output[:MAX_MSG_LEN]}\n```")

    @app_commands.command(name="disk", description="Check disk space on a server")
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    @app_commands.autocomplete(server=server_autocomplete)
    async def disk(self, interaction: discord.Interaction, server: str) -> None:
        self.ensure_server_access(interaction, server)
        await interaction.response.defer()
        output = await asyncio.to_thread(self.state.ssh_manager.execute_command, server, "df -h")
        await interaction.followup.send(f"**Disk Space on `{server}`:**\n```\n{output[:MAX_MSG_LEN]}\n```")

    @app_commands.command(name="update", description="Check and install updates on a server")
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    @app_commands.autocomplete(server=server_autocomplete)
    ...
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    @app_commands.autocomplete(server=server_autocomplete)
    async def process(self, interaction: discord.Interaction, server: str, search: str) -> None:
        self.ensure_server_access(interaction, server)
        await interaction.response.defer()
        cmd = f"ps aux | grep -i -e {shlex.quote(search)} | grep -v grep"
        output = await asyncio.to_thread(self.state.ssh_manager.execute_command, server, cmd)
        await interaction.followup.send(f"**Processes on `{server}` (Search: '{search}'):**\n```\n{output[:MAX_MSG_LEN]}\n```")

    @app_commands.command(name="service", description="Control a systemd service on a server")
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    @app_commands.autocomplete(server=server_autocomplete)
    ...
    async def service(self, interaction: discord.Interaction, server: str, action: str, name: str) -> None:
        self.ensure_server_access(interaction, server)
        await interaction.response.defer()
        self.state.audit_log(interaction.user.id, interaction.user.name, "service", f"Action: {action} | Service: {name} | Server: {server}")
...
class DockerGroup(app_commands.Group, name="docker", description="Manage Docker containers"):
    def __init__(self, bot: DiscoBunty):
        super().__init__()
        self.bot = bot
        self.state = bot.state
    ...
    async def server_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
        ...
        cog = self.bot.get_cog("ServerManagementCog")
        if cog:
            return await cog._generic_autocomplete(interaction, current, fetcher)
        return []

    async def container_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
        async def fetcher():
            return await asyncio.to_thread(self.state.ssh_manager.get_containers, interaction.namespace.server)

        cog = self.bot.get_cog("ServerManagementCog")
        if cog:
            return await cog._generic_autocomplete(interaction, current, fetcher, require_server=True)
        return []

    @app_commands.command(name="ps", description="List containers on a server")
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    @app_commands.autocomplete(server=server_autocomplete)
    async def docker_ps(self, interaction: discord.Interaction, server: str, all: bool = True) -> None:
        self.ensure_server_access(interaction, server)
        await interaction.response.defer()
    ...
    @app_commands.command(name="control", description="Start, stop, or restart a container")
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    ...
    async def docker_control(self, interaction: discord.Interaction, server: str, action: str, container: str) -> None:
        ...
        output = await asyncio.to_thread(self.state.ssh_manager.container_action, server, container, action)
    ...
    async def docker_logs(self, interaction: discord.Interaction, server: str, container: str, lines: int = 50, search: str | None = None) -> None:
        ...
        await interaction.response.defer()
        lines = min(max(1, lines), 100)
        output = await asyncio.to_thread(self.state.ssh_manager.get_container_logs, server, container, lines, search)
        await interaction.followup.send(f"**Logs for `{container}` on `{server}` (Last {lines} lines):**\n```\n{output[:MAX_MSG_LEN]}\n```")

    @app_commands.command(name="details", description="View container image, IP, and ports")
    @app_commands.check(lambda interaction: check_permissions(interaction.client.state, interaction.user))
    @app_commands.autocomplete(server=server_autocomplete, container=container_autocomplete)
    async def docker_details(self, interaction: discord.Interaction, server: str, container: str) -> None:
        self.ensure_server_access(interaction, server)
        await interaction.response.defer()
        output = await asyncio.to_thread(self.state.ssh_manager.get_container_details, server, container)
